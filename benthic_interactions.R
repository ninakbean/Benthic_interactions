...
master <- coral%>%
  ...
  mutate("Sponge_area" = replace_na(Sponge_area,0))%>% #b/c summing these columns doesn't work with NAs
  mutate("Algae_area" = replace_na(Algae_area,0))%>% #b/c summing these columns doesn't work with NAs
...
master.onecoral <- master%>%
  group_by(Date, Transect, Quadrat, ID, Type, Coral, 
           Sponge_inter,
           Inferred_algae,
           use_for_size, CoralArea_category)%>%
